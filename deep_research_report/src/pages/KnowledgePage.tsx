...
const STATUS_MAP: Record<string, { label: string; cls: string }> = {
  ready: { label: '已索引', cls: 'badge-success' },
  processing: { label: '处理中', cls: 'badge-warning' },
  error: { label: '索引失败', cls: 'badge-error' },
};
...
const ENDPOINTS: Record<string, { method: string; path: string; desc: string }[]> = {
  reports: [
    ...
    { method: 'DELETE', path: '/api/v1/reports/:id', desc: '删除指定报告' },
  ],
  knowledge: [
    { method: 'GET', path: '/api/v1/knowledge/documents', desc: '获取知识库文档列表' },
    { method: 'POST', path: '/api/v1/knowledge/documents', desc: '上传文档至知识库' },
    { method: 'GET', path: '/api/v1/knowledge/documents/:id', desc: '获取文档详情与索引状态' },
    { method: 'DELETE', path: '/api/v1/knowledge/documents/:id', desc: '从知识库中移除文档' },
  ],
};
...
export default function KnowledgePage() {
  ...
  const currentEPs = ENDPOINTS[apiTab] || [];

  return (
    <div>
      <div className="view-kb-header">
        <h1 className="view-title">知识库</h1>
      ...
      <div className="kb-table-wrap">
        <table className="table">
          ...
          <tbody>
            {docs.length === 0 ? (
              <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--color-text-disabled)', padding: 'var(--space-8)' }}>加载中...</td></tr>
            ) : (
              docs.map((doc) => (
                <tr key={doc.id}>
                  <td><span style={{ fontWeight: 500, color: 'var(--color-text-primary)' }}>{doc.title}</span></td>
                  <td><code>{doc.source.toUpperCase()}</code></td>
                  <td><span className={'badge ' + (STATUS_MAP[doc.status]?.cls || 'badge-neutral')}>{STATUS_MAP[doc.status]?.label || doc.status}</span></td>
                  <td style={{ color: 'var(--color-text-secondary)' }}>{doc.createdAt.slice(0, 10)}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <div className={'api-panel' + (apiOpen ? ' open' : '')}>
        <div className="api-panel-header" onClick={() => setApiOpen(!apiOpen)} role="button" tabIndex={0} aria-expanded={apiOpen}>
          <span className="api-panel-title">API 接口参考</span>
          <svg className="api-panel-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: 16, height: 16 }}>
        ...
        <div className="api-panel-body">
          <div className="api-tabs">
            {['reports', 'knowledge'].map((tab) => (
              <button key={tab} className={'api-tab' + (apiTab === tab ? ' active' : '')} onClick={() => setApiTab(tab)}>
